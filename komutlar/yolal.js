...
var prefix = ayarlar.prefix;
 ...
 if (!message.guild.member(user).bannable) return message.channel.send(`Bu kişiyi sunucudan yasaklayamıyorum çünkü \`benden daha yüksek bir role sahip\` ya da \`bana gerekli yetkileri vermedin\`.`);
  ...
  .setAuthor('YOLSUZLARA YOL VERMEK BİZİM İŞİMİZ', message.author.displayAvatarURL())
  .setThumbnail(client.user.avatarURL())
  .setColor('RANDOM')
  .addField('Bilgiler:', `
  **\nSeni Gidi Orospu Maymun:<@${user.id}> ** 
  **\nsenin amına <@${message.author.id}> koymuş daha çok uzun yolun var hadi bb **
  **\nSebebi:${reason}\**
  `)
  .setImage('https://media4.giphy.com/media/f4bQadlNq3RvdSKx04/200.gif')
